...
public class RunnerCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("runner")
                ...
                .then(CommandManager.argument("targets", EntityArgumentType.players())
                        .requires(source -> source.isExecutedByPlayer() && ManhuntMod.checkLeaderPermission(source.getPlayer(), "libreh.manhunt.runner") || !source.isExecutedByPlayer())
    ...
    private static int selfRunner(ServerCommandSource source) {
        if (source.isExecutedByPlayer() && (ManhuntConfig.CONFIG.getRolePreset() == 1 || ManhuntMod.checkLeaderPermission(source.getPlayer(), "libreh.manhunt.runner"))) {
            ...
            if (!player.isTeamPlayer(runnersTeam)) {
                ...
                server.getPlayerManager().broadcast(Text.translatable("chat.manhunt.joined_team",
                                ...
                                ,
                                Text.translatable("role.manhunt.runners").formatted(ManhuntConfig.CONFIG.getRunnersColor())),
                        false);
            } else {
