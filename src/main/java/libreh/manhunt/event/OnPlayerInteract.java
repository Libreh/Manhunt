...
public class OnPlayerInteract {
    ...
    public static ActionResult useItem(PlayerEntity player, World world, Hand hand) {
        ...
        if (gameState == GameState.PREGAME) {
            ...
            if (customData != null && customData.copyNbt().getBoolean("Tracker") && ManhuntConfig.CONFIG.getTrackerType() == 2 || SettingsGui.TRACKER_TYPE.get(player.getUuid()) == 2 && player.isTeamPlayer(huntersTeam) && !OnGameTick.allRunners.isEmpty() && !player.isSpectator() && !player.getItemCooldownManager().isCoolingDown(stack)) {
