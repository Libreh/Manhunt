    ...
    public static void playerJoin(ServerPlayNetworkHandler handler) {
        ...
        if (!SettingsGui.CUSTOM_TITLES.containsKey(player.getUuid())) {
            ...
            SettingsGui.CUSTOM_TITLES.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "custom_titles").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "custom_titles").get());
            SettingsGui.CUSTOM_SOUNDS.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "custom_sounds").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "custom_sounds").get());
            SettingsGui.CUSTOM_PARTICLES.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "custom_particles").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "custom_particles").get());
            var trackerType = 1;
            if (dataContainer.get().get(JavaTypes.INT,
                    "tracker_type").isPresent()) {
                trackerType = dataContainer.get().get(JavaTypes.INT,
                        "tracker_type").get();
            }
            SettingsGui.TRACKER_TYPE.put(player.getUuid(), trackerType);
            SettingsGui.NIGHT_VISION.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "night_vision").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "night_vision").get());
            SettingsGui.FRIENDLY_FIRE.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "friendly_fire").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "friendly_fire").get());
            SettingsGui.BED_EXPLOSIONS.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "bed_explosions").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "bed_explosions").get());
            SettingsGui.LAVA_PVP_IN_NETHER.put(player.getUuid(), dataContainer.get().get(JavaTypes.BOOL,
                    "lava_pvp_in_nether").isPresent() && dataContainer.get().get(JavaTypes.BOOL,
                    "lava_pvp_in_nether").get());
        }
    }
