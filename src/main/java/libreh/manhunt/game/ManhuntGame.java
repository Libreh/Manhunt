...
import libreh.manhunt.command.game.CoordsCommand;
import libreh.manhunt.command.game.DurationCommand;
...
import libreh.manhunt.mixin.world.IServerWorld;
        ...
        if (OnGameTick.waitForRunner || OnGameTick.runnerHasStarted) {
            for (ServerPlayerEntity serverPlayer : server.getPlayerManager().getPlayerList()) {
                if (serverPlayer.isTeamPlayer(huntersTeam)) {
                    serverPlayer.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(0);
                    serverPlayer.getAttributeInstance(EntityAttributes.JUMP_STRENGTH).setBaseValue(0);
                    serverPlayer.getAttributeInstance(EntityAttributes.BLOCK_BREAK_SPEED).setBaseValue(0);
