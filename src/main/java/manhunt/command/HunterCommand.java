    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal("hunter")
                ...
                .then(argument("targets", EntityArgumentType.players())
                        .executes(context -> setHunter(context.getSource(), EntityArgumentType.getPlayers(context, "targets")))
                )
        );
    }

    private static int setHunter(ServerCommandSource source, Collection<ServerPlayerEntity> players) {
        for (ServerPlayerEntity player : players) {
            ...
            player.getServer().getPlayerManager().broadcast(Text.translatable("manhunt.chat.role", Text.literal(player.getName().getString()).formatted(config.getHuntersColor()), Text.translatable("manhunt.hunter").formatted(config.getHuntersColor())), false);
        }
